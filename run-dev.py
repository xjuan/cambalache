...
def compile_schemas(schema_xml):
    ...
    if not os.path.exists(schema) or os.path.getmtime(schema) < os.path.getmtime(schema_xml):
        ...
        GLib.spawn_sync(".", [glib_compile_schemas, schemadir], None, GLib.SpawnFlags.DEFAULT, None, None)
...
def update_mime(mime_xml):
    ...
    if not os.path.exists(mime) or os.path.getmtime(mime) < os.path.getmtime(mime_xml):
        ...
        GLib.spawn_sync(".", [update_mime_database, mimedir], None, GLib.SpawnFlags.DEFAULT, None, None)
...
def compile_private():
    ...
    for v, pkg in [("3", "gtk+-3.0"), ("4", "gtk4")]:
        ...
        os.system(
            f"g-ir-scanner -i Gtk-{v}.0 -n CambalachePrivate --nsversion={v}.0 --identifier-prefix=cmb_private -L {privatedir} \
...
""",
    ...
    configure_file(
        "cambalache/merengue/merengue.in",
        "cambalache/merengue/merengue",
        {"PYTHON": GLib.find_program_in_path("python3"), "merenguedir": os.path.abspath("cambalache")},
    )
